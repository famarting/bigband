...
//   - calls submit_run to fire one-off tasks from Slack messages
//   - calls submit_run with parent_session_id to follow up in threads
//
// Configuration lives at ~/.bigband-tasks/extensions/bigband-slack/config.yaml.
// By default the binary mirrors NOTHING — only tasks listed in mirror rules
...
func newRoot() *cobra.Command {
	root := &cobra.Command{
		...
		Long: `bigband-slack mirrors completed bigband task runs into Slack threads
...
~/.bigband-tasks/extensions/bigband-slack/config.yaml — by default nothing
is mirrored until you opt tasks in via mirror rules.`,
		SilenceUsage: true,
	}
