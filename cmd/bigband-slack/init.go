...
By default neither file is overwritten if it already exists. Use:
  ...
  --force            equivalent to --force-config --force-manifest`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("                 groups:history, groups:read, app_mentions:read,")
			fmt.Println("                 reactions:write, users:read")
			fmt.Println("     (mirror-only setups need just chat:write — the rest is the inbound path)")
