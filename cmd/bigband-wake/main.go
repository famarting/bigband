...
// scheduled bigband task. The goal: a closed-lid MacBook on AC wakes itself a
// minute before each task fires, runs it, and goes back to sleep — so cron
...
//
//   - subscribes to task_run.completed / config.reloaded events to know when
//     to recompute the wake schedule
//   - calls Status to read each task's next_run
//   - shells out to `sudo -n pmset schedule …` to register / cancel wakes
//
// Configuration lives at ~/.bigband-tasks/extensions/bigband-wake/config.yaml.
...
cron schedule so the laptop wakes itself moments before each task fires.
...
~/.bigband-tasks/extensions/bigband-wake/config.yaml.
