...
func TestEffectiveEnvNilWhenUnset(t *testing.T) {
	c := &config.Config{}
	if got := c.EffectiveEnv(&config.Job{Name: "a"}); got != nil {
		t.Fatalf("want nil when neither defaults nor entry set env, got %v", got)
	}
}

func TestEffectiveEnvFromDefaultsOnly(t *testing.T) {
	c := &config.Config{Defaults: config.Defaults{Env: map[string]string{"A": "1"}}}
	got := c.EffectiveEnv(&config.Job{Name: "a"})
	if len(got) != 1 || got["A"] != "1" {
		t.Fatalf("want A=1 inherited from defaults, got %v", got)
	}
}

func TestEffectiveEnvEntryOverridesDefaultsPerKey(t *testing.T) {
...
func TestEffectiveEnvDoesNotMutateDefaults(t *testing.T) {
	defs := map[string]string{"A": "1"}
	c := &config.Config{Defaults: config.Defaults{Env: defs}}
	_ = c.EffectiveEnv(&config.Job{Name: "a", Env: map[string]string{"A": "2", "B": "3"}})
...
`
	...
	if len(c.Jobs) != 1 {
		t.Fatalf("want 1 entry, got %d", len(c.Jobs))
	}
	got := c.EffectiveEnv(c.Jobs[0])
	if got["OPENAI_API_KEY"] != "sk-test" {
		t.Errorf("env: block did not parse, got %v", got)
	}
	if got["SHARED"] != "from-entry" {
		t.Errorf("entry env: should override defaults, got %q", got["SHARED"])
	}
}
